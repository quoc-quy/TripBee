...
import { getImageUrl } from '../../../utils/utils' // THÊM IMPORT NÀY
...
export default function FormTourScreen() {
  ...
  const { data: tourDetail, isLoading: loadingTour } = useQuery<TourDetailAdmin>({
  ...
  // ==== Query khuyến mãi ====
  const { data: promotions = [], isLoading: loadingPromotions } = useQuery<PromotionSimple[]>({
  ...
  const { data: completedTours = [], isLoading: loadingCompletedTours } = useQuery<SimpleTour[]>({
    queryKey: ['admin-tours-completed-simple'],
    queryFn: () => tourAdminApi.getCompletedToursSimple().then((res) => res.data),
    enabled: !isEdit
  })
  ...
  const fillFromTourDetail = (detail: TourDetailAdmin, opts?: { forceActive?: boolean }) => {
    ...
    const mappedDestinations: DestinationAdmin[] = tourDestinations
      ...
      .filter((d): d is { id: string; nameDes: string; region: string } => !!d)
      .map((d) => ({
        destinationID: d.id,
        nameDes: d.nameDes,
        region: d.region
    ...
    setForm({
      ...
      priceAdult: typeof detail.priceAdult === 'number' ? detail.priceAdult : '',
      priceChild: typeof detail.priceChild === 'number' ? detail.priceChild : '',
      minGuests: typeof detail.minParticipants === 'number' ? detail.minParticipants : '',
      maxGuests: typeof detail.maxParticipants === 'number' ? detail.maxParticipants : '',
      imageURL: detail.imageURL ?? '',
      durationDays: typeof detail.durationDays === 'number' ? detail.durationDays : '',
      durationNights: typeof detail.durationNights === 'number' ? detail.durationNights : '',
      description: detail.description ?? '',
      tourTypeId: detail.tourType?.id ?? '',
      status: opts?.forceActive ? 'ACTIVE' : (detail.status?.toUpperCase() ?? 'ACTIVE'),
  ...
  const handleChange = (
    ...
    const numericFields = ['minGuests', 'maxGuests', 'durationDays', 'durationNights']
  ...
  const handleRemoveDestination = (id: string) => {
    setSelectedDestinations((prev) => prev.filter((d) => d.destinationID !== id))
    setForm((prev) => ({
      ...prev,
  ...
  const handleRemovePromotion = (id: string) => {
    setSelectedPromotions((prev) => prev.filter((p) => p.promotionID !== id))
  }
  ...
  const handleRemoveItineraryRow = (index: number) => {
    setItineraries((prev) =>
      prev.filter((_, i) => i !== index).map((it, idx) => ({ ...it, dayNumber: idx + 1 }))
    )
  }
  ...
  const validate = (): FormErrors => {
    ...
    const checkPositive = (value: number | '', field: keyof TourForm, label: string) => {
    ...
    if (!form.startDate) {
      ...
      if (isNaN(start.getTime())) {
        ...
        if (start < minStart) {
          newErrors.startDate = 'Ngày khởi hành phải sau ngày hiện tại ít nhất 1 tháng'
        }
      }
    }
  ...
  if (isEdit && loadingTour) {
    return <div className="p-8 text-base">Đang tải dữ liệu tour</div>
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {!isEdit && (
          <section className="bg-white rounded-2xl shadow-sm p-6 space-y-3 border border-gray-100">
            <h2 className="text-lg font-semibold text-gray-800">Mở lại tour đã hoàn thành</h2>
            <div className="mt-3 space-y-2">
              <Select
                ...
                value={reopenTourOptions.find((opt) => opt.value === selectedReopenTourId) || null}
                onChange={(opt: any) => {
        ...
        <section className="bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1">Tên tour</label>
              {errors.title && <p className="text-xs text-red-500 mb-1">{errors.title}</p>}
              <input
                name="title"
                value={form.title}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1">Loại tour</label>
              {errors.tourTypeId && (
                <p className="text-xs text-red-500 mb-1">{errors.tourTypeId}</p>
              )}
              <select
                name="tourTypeId"
        ...
        <section className="bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-gray-100">
          <h2 className="text-lg font-semibold text-gray-800">Lịch trình & thời lượng</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1">Ngày khởi hành</label>
              {errors.startDate && <p className="text-xs text-red-500 mb-1">{errors.startDate}</p>}
              <input
                type="date"
                name="startDate"
            ...
            <div>
              ...
              {errors.endDate && <p className="text-xs text-red-500 mb-1">{errors.endDate}</p>}
              <input
                type="date"
                name="endDate"
                value={form.endDate}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1">Số ngày</label>
              {errors.durationDays && (
                <p className="text-xs text-red-500 mb-1">{errors.durationDays}</p>
              )}
              <input
                type="number"
                min={0}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1">Số đêm</label>
              {errors.durationNights && (
                <p className="text-xs text-red-500 mb-1">{errors.durationNights}</p>
              )}
              <input
                type="number"
                min={0}
        ...
        {/* Giá & số lượng */}
        <section className="bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-gray-100">
          <h2 className="text-lg font-semibold text-gray-800">Giá & số lượng</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              {errors.priceAdult && (
                <p className="text-xs text-red-500 mb-1">{errors.priceAdult}</p>
              )}
              <input
                type="text"
                name="priceAdult"
            ...
            <div>
              ...
              {errors.priceChild && (
                <p className="text-xs text-red-500 mb-1">{errors.priceChild}</p>
              )}
              <input
                type="text"
                name="priceChild"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              {errors.minGuests && <p className="text-xs text-red-500 mb-1">{errors.minGuests}</p>}
              <input
                type="number"
                min={0}
                name="minGuests"
            ...
            <div>
              ...
              {errors.maxGuests && <p className="text-xs text-red-500 mb-1">{errors.maxGuests}</p>}
              <input
                type="number"
                min={0}
                name="maxGuests"
        ...
        {/* Khu vực & điểm đến */}
        <section className="bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-gray-100">
          <h2 className="text-lg font-semibold text-gray-800">Khu vực & điểm đến</h2>

          {destinationsError && <p className="text-sm text-red-500">Lỗi tải điểm đến</p>}

          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1">Miền</label>
              {errors.region && <p className="text-xs text-red-500 mb-1">{errors.region}</p>}
              <select
                name="region"
                value={form.region}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1">Điểm đến</label>
              {errors.destinationId && (
                <p className="text-xs text-red-500 mb-1">{errors.destinationId}</p>
              )}
              <select
                name="destinationId"
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            <span className="text-sm font-semibold text-gray-900">{d.nameDes}</span>
                        ...
                        <button
                          type="button"
                          onClick={() => handleRemoveDestination(d.destinationID)}
        ...
        {/* Khuyến mãi */}
        <section className="bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-gray-100">
          <h2 className="text-lg font-semibold text-gray-800">Khuyến mãi áp dụng</h2>
        ...
        <section className="bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-gray-100">
          ...
          {itineraries.length === 0 ? (
            <p className="text-sm text-gray-500">Chưa có lịch trình, bấm “Thêm ngày” để bắt đầu.</p>
          ) : (
                ...
                <tbody>
                  {itineraries.map((it, index) => (
                    <tr key={index} className="border-t border-gray-200">
                      <td className="px-4 py-2 text-center font-semibold">Ngày {index + 1}</td>
        ...
        {/* Mô tả tour */}
        <section className="bg-white rounded-2xl shadow-sm p-6 space-y-3 border border-gray-100">
          <h2 className="text-lg font-semibold text-gray-800">Mô tả tour</h2>
          {errors.description && <p className="text-xs text-red-500 mb-1">{errors.description}</p>}
          <textarea
            name="description"
        ...
        <section className="bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-gray-100">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-gray-800">Hình ảnh & trạng thái</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              <label className="block text-sm font-medium text-gray-600">Ảnh bìa tour</label>
              ...
              {uploadingImage && <p className="text-xs text-gray-500">Đang tải ảnh lên...</p>}

              {form.imageURL && !uploadingImage && (
                <div className="mt-1">
                  <p className="text-xs text-gray-500 mb-1">Xem trước ảnh bìa</p>
                  <div className="relative w-full h-44 rounded-2xl overflow-hidden border border-gray-200 bg-gray-50 group">
                    <img
                      src={getImageUrl(form.imageURL)} // ĐÃ CẬP NHẬT Ở ĐÂY
                      alt="Preview"
            ...
            {/* Cột phải: trạng thái */}
            <div className="space-y-3">
              <label className="block text-sm font-medium text-gray-600">Trạng thái tour</label>
              <select
                name="status"
                value={form.status}
                onChange={handleChange}
