...
export default function ManageTourScreen() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      tourAdminApi.getAllTours(queryParams as TourListAdminParams).then((res) => res.data),
    placeholderData: keepPreviousData,
  });

  // === Lấy loại tour ===
  // const { data: tourTypes = [], isLoading: isLoadingTypes } = useQuery({
  //   queryKey: ["tourTypes"],
  //   queryFn: () => tourTypeApi.getTourTypes().then((res) => res.data ?? res),
  // });
    ...
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Danh sách tour hiện có trong hệ thống</p>
        </div>
        <button className="bg-blue-600 hover:bg-blue-700 text-white text-base font-semibold px-6 py-3 rounded-lg shadow-lg transition-all active:scale-95">
          + Tạo tour mới
        </button>
      </div>
      ...
      <div className="bg-white shadow-md rounded-xl p-5 flex flex-wrap items-center gap-4 mb-6">
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter") {
              const newParams = { ...queryParams, search: e.currentTarget.value, page: 0 };
              setSearchParams(newParams as any);
            }
          }}
        ...
        <select
          ...
          onChange={(e) => {
            const newParams = { ...queryParams, tour_type_id: e.target.value || undefined, page: 0 };
            setSearchParams(newParams as any);
          }}
        >
        ...
        <select
          ...
          onChange={(e) => {
            const newParams = { ...queryParams, status: e.target.value || undefined, page: 0 };
            setSearchParams(newParams as any);
          }}
        >
          ...
          <tbody className="text-gray-800">
            {isLoading ? (
              ...
              tours.map((tour: any) => {
                ...
                return (
                  <tr key={tour.tourID} className="border-b border-gray-100 hover:bg-gray-50 transition-all">
                    <td className="px-5 py-4 text-center text-sm">{tour.tourID}</td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-4">
                        <img src={tour.imageURL} alt={tour.title} className="w-20 h-20 rounded-xl object-cover" />
                        <div>
                          <p className="font-semibold text-gray-900 text-sm">{tour.title}</p>
                          <p className="text-gray-500 text-sm">{tour.tourTypeName}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4 text-center text-sm">
                      <span className={`px-3 py-1.5 rounded-full font-medium ${color}`}>{label}</span>
                    </td>
                    <td className="px-5 py-4 text-center text-sm">
                      <div className="inline-flex gap-2">
                        <button className="border border-blue-500 text-blue-500 rounded-lg p-2 hover:bg-blue-50">
                          <Edit2 size={16} />
                        </button>
